...
const CompareCollegesPage: React.FC<CompareCollegesPageProps> = ({ onNavigate }) => {
    ...
    return (
        <div className="bg-[#f8f9fc] min-h-screen flex flex-col items-center w-full font-sans pb-16 pt-6">
            <style>{`
        /* The exact fixed-size hero section */
        .hero-section {
            background-color: #536DFE;
            width: 1381px;
            max-width: 95vw; /* Added max-width for better responsiveness on smaller screens */
            height: 655px;
            position: relative;
            overflow: visible;
            display: flex;
        ...
        /* To keep the SVG clipped to the rounded corners while allowing dropdowns to overflow */
        .hero-bg-container {
            position: absolute;
            inset: 0;
            border-radius: 20px;
        ...
        /* Custom styles to refine the design */
        .input-shadow {
      ...
      `}</style>
            ...
            {/* Hero Section Wrapper with exact W & H */}
