...
const EventDetailsPage: React.FC<EventDetailsPageProps> = ({ onNavigate }) => {
  const location = useLocation();
  const state = location.state as { id?: string } | null;
  const eventId = state?.id || "1";

  const event = getEventById(eventId);
  const related = event ? getRelatedEvents(eventId, event.category) : [];
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 lg:pb-14 bg-white min-h-screen">
      ...
      <div className="mt-16 lg:mt-24 border-t border-gray-100 pt-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => onNavigate("eventsPage")} className="text-blue-600 text-sm font-medium hover:underline">
            View All
          </button>
        </div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="flex-1 border border-gray-200 text-gray-700 text-[13px] font-semibold py-2.5 rounded-lg hover:bg-gray-50 transition-colors"
                    >
                      Details
                    </button>
                    <button
