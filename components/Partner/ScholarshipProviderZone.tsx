...
const services: ServiceCard[] = [
  {
    ...
    description: "Steamline and track all scholarship applications in one place.",
  ...
  },
  {
    ...
    description: "Filter and verify candidates based on your specific criteria.",
    icon: "fa-user-check",
    blobClass: "bg-[#fff7ed]",
  ...
  },
  {
    ...
    description: "Handle student questions directly through our messaging system.",
    icon: "fa-headset",
    blobClass: "bg-[#f3e8ff]",
  ...
  },
  {
    ...
    description: "Build trust with students through a verified scholarship badge.",
    icon: "fa-certificate",
    blobClass: "bg-[#fef2f2]",
  ...
  },
  {
    ...
    description: "Get detailed insights into your scholarship reach and impact.",
    icon: "fa-chart-pie",
    blobClass: "bg-[#eef2ff]",
  ...
  },
  {
    ...
    description: "Maintain a database of past and present scholarship recipients.",
    icon: "fa-users-gear",
    blobClass: "bg-[#fff7ed]",
  ...
  },
  {
    ...
    description: "Connect with students from across the globe or specific regions.",
    icon: "fa-globe",
    blobClass: "bg-[#fef2f2]",
    iconClass: "text-red-500",
  },
...
const categories: FeatureCategory[] = [
  {
    ...
    features: [
      { name: "Scholarship Indexing", free: "Standard Listing", standard: "Priority Listing", premium: "Top Placement & Featured" },
      { name: "Verified Provider Badge", free: false, standard: true, premium: true },
      { name: "Brand Page", free: "Basic Information", standard: "Detailed Portfolio", premium: "Premium Media-rich Page" },
      { name: "SEO Optimization", free: false, standard: "Standard SEO", premium: "Advanced SEO & Link-building" },
    ],
  },
  {
    name: "Application & Management",
    features: [
      { name: "Applicant Filtering", free: "Basic (GPA, Region)", standard: "Advanced (Skills, Need)", premium: "AI-based Matching" },
      { name: "Document Collection", free: "Up to 3 docs/applicant", standard: "Unlimited docs", premium: "Managed Verification" },
      { name: "In-Portal Messaging", free: false, standard: "Standard Chat", premium: "Priority Chat & Notifications" },
      { name: "Workflow Management", free: "Basic status update", standard: "Custom workflows", premium: "Automated Workflows" },
    ],
  },
  {
    name: "Analytics & Reporting",
    features: [
      { name: "Applicant Demographics", free: "Basic stats", standard: "Detailed reports", premium: "Advanced BI Dashboards" },
      { name: "Financial Tracking", free: false, standard: "Disbursement log", premium: "Full financial management" },
      { name: "Impact Assessment", free: false, standard: "Annual report", premium: "Real-time impact tracking" },
    ],
  },
];
...
const ScholarshipProviderZone: React.FC<ScholarshipProviderZoneProps> = ({ onNavigate }) => {
  ...
  const handleProviderRegister = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!token || !user) throw new Error("Invalid registration response from server");
    ...
    <div className="text-gray-800 antialiased overflow-x-hidden bg-[#fcfcfc] min-h-screen font-sans">
        ...
        <header className="w-full max-w-7xl mx-auto px-6 py-6 lg:py-8 flex items-center justify-between text-white relative">
          ...
          <nav className="hidden lg:flex lg:flex-1 justify-center items-center gap-8 text-[15px] font-medium relative">
            ...
            <div className="relative" ref={contactDropdownRef}>
              <button type="button" onClick={() => setIsContactOpen((prev) => !prev)} className="hover:text-white/80 transition-colors focus:outline-none flex items-center gap-1">
                Contact us
                  ...
                  <div className="relative bg-white rounded-[16px] p-2 border border-gray-100 shadow-xl">
                    <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-[#f8f9fa] transition-colors group mb-1">
                      <div className="w-16 h-16 rounded-full overflow-hidden flex-shrink-0 bg-gray-100 border border-gray-200">
                        <img src="https://i.pravatar.cc/150?img=33" alt="Advisor" className="w-full h-full object-cover" />
                      </div>
                      <div className="flex flex-col flex-grow pt-0.5">
                        <h3 className="text-[16px] font-semibold text-[#202124]">Emma Wilson</h3>
                        <p className="text-[13px] text-[#6366F1] font-medium mt-1">Partnerships Manager</p>
                        <div className="mt-2 space-y-1.5">
                          <a href="mailto:emma.wilson@studsphere.com" className="flex items-center gap-2 text-[13px] text-[#5f6368] hover:text-[#6366F1]">
                            <i className="fa-regular fa-envelope"></i> emma.wilson@studsphere.com
                          </a>
                        </div>
                      </div>
                    </div>
        ...
        <main className="flex-1 flex items-center justify-center w-full pb-12 lg:pb-20 pt-4">
          <div className="max-w-7xl w-full mx-auto px-6 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-24">
            <div className="flex-1 text-white w-full max-w-2xl text-center lg:text-left pt-6 lg:pt-0">
              <h1 className="text-4xl lg:text-[3.5rem] font-bold leading-[1.15] mb-6">Empower the Next Generation – List Your Scholarships</h1>
              <p className="text-white/90 text-lg lg:text-xl leading-relaxed max-w-lg mx-auto lg:mx-0 font-light">Join Nepal's largest scholarship network and connect with deserving students nationwide.</p>
            </div>
                ...
                {heroTab === "advertise" ? (
                  <div className="fade-in">
                    <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                      <input type="text" placeholder="Full name" className="w-full px-4 py-3.5 border border-gray-200 rounded-xl focus:border-[#6366F1] outline-none" />
                      <input type="tel" placeholder="Contact number" className="w-full px-4 py-3.5 border border-gray-200 rounded-xl focus:border-[#6366F1] outline-none" />
                      <input type="email" placeholder="Organization email" className="w-full px-4 py-3.5 border border-gray-200 rounded-xl focus:border-[#6366F1] outline-none" />
                      <button type="submit" className="w-full mt-4 bg-white border-2 border-[#6366F1] text-[#6366F1] font-semibold py-3.5 rounded-xl hover:bg-indigo-50">Request callback</button>
                    </form>
                  </div>
                ) : authSubTab === "login" ? (
                  <div className="fade-in">
                    <form onSubmit={handleProviderLogin} className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        />
                        <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" onClick={() => setShowLoginPassword((p) => !p)}>
                          <i className={`fa-solid ${showLoginPassword ? "fa-eye-slash" : "fa-eye"}`}></i>
                        </button>
                      </div>
                      <div className="flex justify-end pt-1">
                        <button type="button" className="text-[14px] text-[#6366F1] font-semibold hover:underline">Forgot password?</button>
                      </div>
                      {authError && <p className="text-sm text-red-500">{authError}</p>}
                      {authSuccess && <p className="text-sm text-emerald-600">{authSuccess}</p>}
                      <button type="submit" disabled={authLoading} className="w-full bg-[#6366F1] text-white font-bold py-3.5 rounded-xl hover:bg-indigo-700 disabled:opacity-50">
                        {authLoading ? "Logging in..." : "Log in"}
                      </button>
                    </form>
                    <div className="mt-8 text-center text-[15px] text-gray-500">
                      New provider? <button type="button" onClick={() => setAuthSubTab("register")} className="text-[#6366F1] font-semibold hover:underline">Create account</button>
                    </div>
                  </div>
                ) : (
                  <div className="fade-in">
                    <form onSubmit={handleProviderRegister} className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        />
                        <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" onClick={() => setShowRegisterPassword((p) => !p)}>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-4 py-3.5 pr-12 bg-[#EEF2F6] rounded-xl"
                        />
                        <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" onClick={() => setShowRegisterConfirmPassword((p) => !p)}>
                          <i className={`fa-solid ${showRegisterConfirmPassword ? "fa-eye-slash" : "fa-eye"}`}></i>
                        </button>
                      </div>
                      {authError && <p className="text-sm text-red-500">{authError}</p>}
                      <button type="submit" disabled={authLoading} className="w-full mt-4 bg-[#6366F1] text-white font-bold py-3.5 rounded-xl hover:bg-indigo-700">
                        {authLoading ? "Creating account..." : "Sign up"}
                      </button>
                    </form>
                    <div className="mt-8 text-center text-[15px] text-gray-500">
                      Already registered? <button type="button" onClick={() => setAuthSubTab("login")} className="text-[#6366F1] font-semibold hover:underline">Log in</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Comprehensive Solutions for Providers</h2>
          <p className="mt-4 text-lg text-gray-500 max-w-2xl mx-auto">Tools built to help you reach the right candidates and manage your scholarship programs efficiently.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service) => (
            <div key={service.title} className="bg-white rounded-xl p-8 shadow-sm hover:translate-y-[-5px] transition-all border border-gray-50">
